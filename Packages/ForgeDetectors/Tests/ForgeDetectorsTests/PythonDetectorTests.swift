...
final class PythonDetectorTests: XCTestCase {
    ...
    func testInstalledReturnsVersionAndPath() async throws {
        let runner = FakeCommandRunner { exe, args in
            if exe.lastPathComponent == "which" && args == ["python3"] {
                return CommandResult(stdout: "/usr/local/bin/python3\n", exitCode: 0)
            }
            if exe.lastPathComponent == "python3" && args == ["--version"] {
                return CommandResult(stdout: "Python 3.12.4\n", exitCode: 0)
            }
            return CommandResult(exitCode: 1)
        }
        let detector = PythonDetector(commandRunner: runner)
        let result = try await detector.detect()
        XCTAssertEqual(result.toolId, .python)
        XCTAssertEqual(result.version, SemVer(major: 3, minor: 12, patch: 4))
        XCTAssertEqual(result.installPath, "/usr/local/bin/python3")
